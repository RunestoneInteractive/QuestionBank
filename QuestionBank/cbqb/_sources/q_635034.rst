.. mchoice:: canterburyqb_635034
    ...
    row sum. Choose the best choice to fill in the blank on **Line 9**  so that    
    ...
    - ``maxSum``    
      ...
      - The local variable `maxVec` is used to keep track of the row number of the    
        maximum row sum seen so far, as the outer loop progresses across all rows in    
        the matrix.  The inner loop computes the sum of all cells in the current row,    
        which is stored in the local variable `sum`.  If the current row's sum is    
        larger than the maximum seen so far, the variable `maxVec` should be updated    
        to be `row`.
    - ``row``    
